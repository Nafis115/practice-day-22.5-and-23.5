...
@login_required
def TransferFundView(request):
    if request.method == "POST":
        form = TransferForm(request.POST)
        if form.is_valid():
            current_user_account = request.user.account
            receiver_account_no = form.cleaned_data['receiver_account_no']  # Assuming this is an integer field
            amount = form.cleaned_data['amount']
            
            try:
                # Retrieve the receiver's account based on the account number
                receiver_account = UserBankAccount.objects.get(account_no=receiver_account_no)
                
                # Perform the funds transfer operation
                result_message = current_user_account.transfer_funds(receiver_account.account_no, amount)
                
                # Send email notification to sender
                send_transaction_email(request.user, amount, "Transfer funds", "transactions/transfer_fund_email.html")
                send_transaction_email(receiver_account.user, amount, "Transfer funds receive", "transactions/transfer_fund_email.html")
